...
const Profile = (props) => {
    const {loggedUser,logged,token}=props;
    const profileId = useParams();
    
    const [user,setUser] = useState();


    async function decodeUser(id){
        try{
            const decodedUser = await axios.get(`/search/userId/${id}`);
    ...
    useEffect(()=>{
        if(props.user){
            ...
            if(Object.keys(profileId).length>0){
                try{
                    decodeUser(profileId.id);
        ...
        <div className='profilePageWrapper text-white'>
            <div className='flex flex-col xx:flex-row xx:gap-4'>
                <div className='profilePageLeftSide flex flex-col pt-3 xx:h-screen bg-[#141313] py-3 rounded-b-xl xx:rounded-none xx:rounded-tr-3xl xx:rounded-br-3xl xx:px-3 w-[100%] xx:max-w-[350px] '>
                    <div>
                        <div>
                            <h1 className='text-center font-semibold text-[1.25em] font-Karla mb-3'>{!profileId.id ? "Manage your profile":""}</h1>
                        ...
                        <div className='profilePosts mt-10 pb-10 flex flex-col gap-20'>
                            {!loaded ? "loading...":
                                ...
                                userPosts.map((post,index)=>{
                                    return <Post token={token} onProfile={true} loggedUser={loggedUser} user={user} key={post._id} {...post} logged={logged} index={index}/>
                                })
                            }
                        </div>
                    </div>
